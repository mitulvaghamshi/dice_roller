...
class _PlaySessionScreenState extends State<PlaySessionScreen>
  ...
  @override
  void dispose() {
    _animation.removeStatusListener(_onAnimationEnd);
    _animation.dispose();
    super.dispose();
  }
        ...
        child: Scaffold(
          ...
          body: Stack(children: [
            ResponsiveScreen(
              ...
              bottomSlot: _BottomSlot(animation: _animation),
            ),
            Positioned.fill(
              child: Visibility(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Consumer<GameState>(builder: (_, state, child) {
        if (isComplete && state.showResults) state.showWinningScreen();
        return LayoutBuilder(builder: (_, constraints) {
          var width = constraints.biggest.width;
          var diceWidth = width ~/ 3.5;
          return Wrap(
            alignment: WrapAlignment.spaceAround,
            children: List.generate(level.dices, (index) {
              return DiceWidget(
                animation: animation,
